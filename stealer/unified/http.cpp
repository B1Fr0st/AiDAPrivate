...
HTTPResponse HTTP::request(const std::string& url, const std::string& method, const std::string& data,
    ...
    HINTERNET session = WinHttpOpen(user_agent.c_str(), WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    ...
    HINTERNET req = WinHttpOpenRequest(connection, s2ws(method).c_str(), components.lpszUrlPath,
    ...
    for (const auto& header : headers) {
        ...
        WinHttpAddRequestHeaders(req, fullHeader.c_str(), -1, WINHTTP_ADDREQ_FLAG_ADD);
    }

    if (!WinHttpSendRequest(req, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
            (LPVOID)(data.empty() ? nullptr : data.c_str()),
            (DWORD)data.length(), (DWORD)data.length(), 0)) {
    ...
    WinHttpQueryHeaders(req, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER,
                        WINHTTP_HEADER_NAME_BY_INDEX, &status_code, &statusCodeSize,
    ...
    WinHttpQueryHeaders(req, WINHTTP_QUERY_RAW_HEADERS_CRLF, WINHTTP_HEADER_NAME_BY_INDEX,
                        &raw_headers[0], &size, WINHTTP_NO_HEADER_INDEX);

    auto response_headers = parse_headers(raw_headers);

    std::vector<char> buffer;
    DWORD bytesRead = 0;
    do {
        buffer.resize(buffer.size() + 4096);
        if (!WinHttpReadData(req, &buffer[buffer.size() - 4096], 4096, &bytesRead)) {
            WinHttpCloseHandle(req);
            WinHttpCloseHandle(connection);
            WinHttpCloseHandle(session);
    ...
    buffer.resize(buffer.size() - 4096 + bytesRead);
