...
namespace dbg_capture {
    ...
    static VOID NTAPI drain_thread_routine(PVOID context)
    {
        ...
        write_immediate_formatted("[SN] dbg_capture::drain_thread_start tid=%llu wait_ms=%lu max_wait_ms=%lu\n",
        ...
        for (;;) {
            ...
            if (flush.bytes != 0) {
                ...
                if (should_log_empty(empty_count)) {
                    write_immediate_formatted("[SN] dbg_capture::drain_thread_idle tid=%llu wait_status=0x%08lx wake=%s empty_count=%llu wait_ms=%lu next_wait_ms=%lu total_bytes=%llu total_flush_us=%llu flush_count=%llu\n",
                        static_cast<unsigned long long>(tid),
                        static_cast<ULONG>(wait_status),
                        wait_reason(wait_status, FALSE),
    ...
    NTSTATUS initialize()
    {
        ...
        write_immediate_formatted("[SN] dbg_capture::initialize_enter irql=%lu pid=%llu tid=%llu ring=%p flush=%p\n",
        ...
        if (!g_ring) {
            write_immediate_formatted("[SN] dbg_capture::initialize_alloc_failed target=ring bytes=%lu status=0x%08lx\n",
                kRingSize,
        ...
        if (!g_flush_buffer) {
            ...
            g_ring = nullptr;
            write_immediate_formatted("[SN] dbg_capture::initialize_alloc_failed target=flush bytes=%lu status=0x%08lx\n",
                kRingSize,
                static_cast<ULONG>(STATUS_INSUFFICIENT_RESOURCES));
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        if (!NT_SUCCESS(st)) {
            ...
            write_immediate_formatted("[SN] dbg_capture::initialize_thread_failed status=0x%08lx ring=%p flush=%p\n",
        ...
        write_immediate_formatted("[SN] dbg_capture::initialize_exit status=0x%08lx ref_status=0x%08lx ring=%p flush=%p thread=%p initialized=%ld\n",
            static_cast<ULONG>(st),
