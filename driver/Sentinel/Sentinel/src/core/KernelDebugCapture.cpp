...
namespace dbg_capture {
    ...
    static void write_immediate_formatted(const char* fmt, ...)
    {
        if (!fmt) return;
    ...
    static VOID NTAPI drain_thread_routine(PVOID context)
    {
        ...
        for (;;) {
            ...
            if (flush.bytes != 0) {
                ...
                if (should_log_flush(flush, flush_count)) {
                    write_immediate_formatted("[SN] dbg_capture::drain_thread_flush tid=%llu wait_status=0x%08lx wake=%s bytes=%lu total_bytes=%llu flush_us=%lu total_flush_us=%llu flush_count=%llu suppressed_flushes=%llu suppressed_bytes=%llu suppressed_flush_us=%llu create=0x%08lx write=0x%08lx next_wait_ms=%lu\n",
                        static_cast<unsigned long long>(tid),
        ...
        if (final_flush.bytes != 0)
            ++flush_count;
        write_immediate_formatted("[SN] dbg_capture::drain_thread_exit tid=%llu final_bytes=%lu total_bytes=%llu empty_count=%llu total_flush_us=%llu flush_count=%llu suppressed_flushes=%llu suppressed_bytes=%llu suppressed_flush_us=%llu create=0x%08lx write=0x%08lx\n",
            static_cast<unsigned long long>(tid),
            final_flush.bytes,
            static_cast<unsigned long long>(total_bytes),
            static_cast<unsigned long long>(empty_count),
