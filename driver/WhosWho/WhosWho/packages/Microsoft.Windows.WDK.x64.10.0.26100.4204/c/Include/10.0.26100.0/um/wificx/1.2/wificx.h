...
{
    RtlZeroMemory(Config, sizeof(WIFI_DEVICE_CONFIG));
    Config->Size = WIFI_STRUCTURE_SIZE(WIFI_DEVICE_CONFIG);
...
typedef struct _WIFI_DEVICE_CAPABILITIES
{
    ...
    UINT8 NumRadios;      // Number of radios (= num of connections on different channels the device can simultaneously Rx and Tx)
...
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_DEVICE_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_DEVICE_CAPABILITIES);
}
...
typedef struct _WIFI_STATION_CAPABILITIES
{
    ...
    ULONG MaxMLOLinksSupported;             // Max number of Links supported by hardware for MLO connections
    BOOLEAN DoNotUseOsReserved;             // Deprecated field, should not be used anymore.
    WDI_MAC_ADDRESS * MLOAddressesList;     // Array of MLO MAC Addresses with MaxMLOLinksSupported elements
    ULONG NumAkmsSupported;                 // Number of Akms supported by the driver
    RSNA_AKM_SUITE * AkmsList;              // Array of Akms with NumAkmsSupported elements
    ULONG NumFIPSCertifiedCipherAlgorithms;                   // Number of cipher algorithms for which the device has received FIPS 140 certification
    WDI_CIPHER_ALGORITHM * FIPSCertifiedCipherAlgorithmsList; // Array of cipher algorithms for which the device has received FIPS 140 certification, with NumFIPSCertifiedCipherAlgorithms elements
    BOOLEAN MSCSSupported;                            // Whether or not MSCS is supported.
    BOOLEAN DSCPToUPMappingSupported;                 // Whether or not DSCP to UP QoS Mapping is supported.
...
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_STATION_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_STATION_CAPABILITIES);
}
...
typedef struct _WIFI_WIFIDIRECT_CAPABILITIES
{
    ...
    BOOLEAN GOon6GHzBandSupported;  // Whether the adapter supports operating a GO on 6GHz band. No = 0
...
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_WIFIDIRECT_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_WIFIDIRECT_CAPABILITIES);
}
...
WIFI_BAND_CAPABILITIES_INIT(
    _Out_ WIFI_BAND_CAPABILITIES * Capabilities
)
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_BAND_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_BAND_CAPABILITIES);
}
...
WIFI_PHY_CAPABILITIES_INIT(
    _Out_ WIFI_PHY_CAPABILITIES * Capabilities
)
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_PHY_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_PHY_CAPABILITIES);
}
...
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_ADAPTER_POWER_OFFLOAD_RSN_REKEY_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_ADAPTER_POWER_OFFLOAD_RSN_REKEY_CAPABILITIES);
...
{
    RtlZeroMemory(Capabilities, sizeof(WIFI_ADAPTER_WAKE_CAPABILITIES));
    Capabilities->Size = WIFI_STRUCTURE_SIZE(WIFI_ADAPTER_WAKE_CAPABILITIES);
}
...
{
    RtlZeroMemory(Demux, sizeof(WIFI_ADAPTER_TX_DEMUX));
    Demux->Size = WIFI_STRUCTURE_SIZE(WIFI_ADAPTER_TX_DEMUX);
