...
#pragma pack(pop)
    ...
    inline NTSTATUS initialize() {
        ...
        if (!_PsSetLoadImageNotifyRoutine || !_PsSetCreateProcessNotifyRoutineEx) {
            WW_LOG("debug_events::initialize: required Ps* routines not resolved (load=%p create_ex=%p)",
        ...
        if (!g_ring) {
            WW_LOG("debug_events::initialize: ring allocation FAILED bytes=%llu", (ULONG64)bytes);
        ...
        if (NT_SUCCESS(img_st)) {
            ...
            WW_LOG("debug_events::initialize: PsSetLoadImageNotifyRoutine OK");
        } else {
            WW_LOG("debug_events::initialize: PsSetLoadImageNotifyRoutine FAILED 0x%08lx", img_st);
        }
        ...
        if (NT_SUCCESS(proc_st)) {
            ...
            WW_LOG("debug_events::initialize: PsSetCreateProcessNotifyRoutineEx OK");
        } else {
            WW_LOG("debug_events::initialize: PsSetCreateProcessNotifyRoutineEx FAILED 0x%08lx", proc_st);
        }
