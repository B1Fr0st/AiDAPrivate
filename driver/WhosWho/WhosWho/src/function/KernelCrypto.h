...
namespace kernel_crypto {
    ...
    static const UINT8 kAesSbox[256] = {
    ...
    static const UINT8 kAesRcon[11] = {
    ...
    __forceinline UINT32 aes_subword(UINT32 x) {
        return (static_cast<UINT32>(kAesSbox[(x >> 24) & 0xFF]) << 24) |
               (static_cast<UINT32>(kAesSbox[(x >> 16) & 0xFF]) << 16) |
               (static_cast<UINT32>(kAesSbox[(x >> 8)  & 0xFF]) << 8)  |
               (static_cast<UINT32>(kAesSbox[x & 0xFF]));
    }
    ...
    __forceinline void aes256_set_encrypt_key(const UINT8 key[AES256_KEY_SIZE], aes256_ctx_t* ctx) {
        ...
        for (ULONG i = AES256_NK; i < AES256_RK_WORDS; ++i) {
            UINT32 t = rk[i - 1];
            if ((i % AES256_NK) == 0) {
                t = aes_subword(aes_rotword(t)) ^ (static_cast<UINT32>(kAesRcon[i / AES256_NK]) << 24);
    ...
    __forceinline void aes256_encrypt_block(const aes256_ctx_t* ctx, const UINT8 in[16], UINT8 out[16]) {
        ...
        for (ULONG round = 1; round < AES256_NR; ++round) {
            UINT8 t[16];
            for (ULONG i = 0; i < 16; ++i) t[i] = kAesSbox[s[i]];
