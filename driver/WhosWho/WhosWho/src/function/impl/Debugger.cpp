...
namespace tctx_diag {
    ...
    __forceinline void log_context(const char* prefix, const char* phase, const char* source, ULONG attempt, NTSTATUS status, NTSTATUS attach_status, BOOLEAN attached, const CONTEXT& ctx, const LARGE_INTEGER& start, const LARGE_INTEGER& freq) {
        WW_LOG("TCTX %s phase=%s source=%s attempt=%u status=0x%08X status_text=%s attach_status=0x%08X attach_status_text=%s attached=%u flags=0x%08X rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rbp=0x%llX rflags=0x%llX dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX core_valid=%u user_sane=%u elapsed_ms=%llu",
            prefix,
            phase,
            source,
            attempt,
...
namespace trapframe_ctx {
    ...
    NTSTATUS read_native_context(PEPROCESS process, PETHREAD thread, HANDLE thread_handle, PCONTEXT ctx, p_thread_ctx request, const char* phase, BOOLEAN private_user_only) {
        ...
        if (private_user_read_available) {
            for (ULONG retry = 0; retry < 4; ++retry) {
                ...
                if (NT_SUCCESS(status) && tctx_diag::user_context_sane(*ctx)) {
                    WW_LOG("TCTX %s read_selected source=ps_private_user_debug status=0x%08X flags=0x%08X attached=0 rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=1 elapsed_ms=%llu",
                        phase,
                        (ULONG)status,
                        ctx->ContextFlags,
                        (unsigned long long)ctx->Rip,
            ...
            for (ULONG retry = 0; retry < 3; ++retry) {
                ...
                if (NT_SUCCESS(status) && tctx_diag::user_context_sane(*ctx)) {
                    WW_LOG("TCTX %s read_selected source=ps_private_user_base status=0x%08X flags=0x%08X attached=0 rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=1 elapsed_ms=%llu",
                        phase,
                        (ULONG)status,
                        ctx->ContextFlags,
                        (unsigned long long)ctx->Rip,
            ...
            WW_LOG("TCTX %s read_private_user_unavailable pid=%u tid=%u user_get=%p state=%ld elapsed_ms=%llu",
                phase,
                request->pid,
                request->tid,
                ssdt_resolver::g_PsGetUserContextThread,
        ...
        if (private_user_only) {
            WW_LOG("TCTX %s read_private_only_fail_closed pid=%u tid=%u status=0x%08X status_text=%s private_user_read_available=%u attempt_count=%lu last_source=%s last_status=0x%08X last_status_text=%s zero_context_seen=%u invalid_user_context_seen=%u previous_mode=%u requestor_mode=%u elapsed_ms=%llu",
                phase,
                request->pid,
                request->tid,
                (ULONG)status,
        ...
        for (ULONG route = 0; route < sizeof(attempts) / sizeof(attempts[0]); ++route) {
            ...
            for (ULONG retry = 0; retry < 4; ++retry) {
                ...
                if (NT_SUCCESS(status) && tctx_diag::user_context_sane(*ctx)) {
                    ...
                    WW_LOG("TCTX %s read_selected source=%s status=0x%08X flags=0x%08X attached=%u rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=1 elapsed_ms=%llu",
                        phase,
                        selected_source,
                        (ULONG)status,
                        selected_flags,
        ...
        WW_LOG("TCTX %s read_exit status=0x%08X status_text=%s selected_source=%s selected_flags=0x%08X selected_attached=%u flags=0x%08X thread_after_found=%u thread_after_state=%lu thread_after_wait_reason=%lu rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=%u dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX elapsed_ms=%llu",
            phase,
            (ULONG)status,
            tctx_diag::ntstatus_text(status),
            selected_source,
    ...
    NTSTATUS write_native_context(PEPROCESS process, PETHREAD thread, HANDLE thread_handle, PCONTEXT ctx, p_thread_ctx request, BOOLEAN private_user_only) {
        ...
        WW_LOG("TCTX set write_route_gate pid=%u tid=%u mask=0x%llX private_user_only=%u private_user_write_available=%u internal_direct=%p ps_set=%p state=%ld previous_mode=%u requestor_mode=%u ctx_thread_handle=%p elapsed_ms=%llu",
            request->pid,
            request->tid,
            (unsigned long long)request->register_mask,
            private_user_only ? 1u : 0u,
        ...
        if (private_user_write_available) {
            ...
            if (NT_SUCCESS(status)) {
                WW_LOG("TCTX set write_selected source=ps_private_user_write status=0x%08X flags=0x%08X rip=0x%llX rsp=0x%llX rflags=0x%llX dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX elapsed_ms=%llu",
                    (ULONG)status,
                    ctx->ContextFlags,
                    (unsigned long long)ctx->Rip,
                    (unsigned long long)ctx->Rsp,
            ...
            WW_LOG("TCTX set write_private_user_unavailable pid=%u tid=%u ps_set=%p internal_direct=%p state=%ld elapsed_ms=%llu",
                request->pid,
                request->tid,
                _PsSetContextThread,
                ssdt_resolver::g_PspSetContextThreadInternal,
    ...
    NTSTATUS set_context(PEPROCESS process, PETHREAD thread, HANDLE thread_handle, p_thread_ctx request, PCONTEXT ctx, BOOLEAN private_user_only) {
        ...
        if (!NT_SUCCESS(status)) {
            WW_LOG("TCTX set base_read_failed status=0x%08X pid=%u tid=%u mask=0x%llX private_user_only=%u",
                (ULONG)status,
        ...
        if (needs_debug && ((ctx->ContextFlags & CONTEXT_DEBUG_REGISTERS) == 0)) {
            WW_LOG("TCTX set base_read_rejected_no_debug pid=%u tid=%u mask=0x%llX flags=0x%08X private_user_only=%u",
                request->pid,
                request->tid,
                (unsigned long long)request->register_mask,
                ctx->ContextFlags,
        ...
        WW_LOG("TCTX set applied pid=%u tid=%u mask=0x%llX flags=0x%08X needs_debug=%u private_user_only=%u rip=0x%llX rsp=0x%llX rflags=0x%llX dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX",
            request->pid,
            request->tid,
            (unsigned long long)request->register_mask,
            ctx->ContextFlags,
...
namespace tctx_system_route {
    ...
    __forceinline NTSTATUS resume_worker_thread(worker_t* work) {
        ...
        WW_LOG("TCTX system_worker_resume_primary route=system_thread reason=%s pid=%u tid=%u status=0x%08X status_text=%s win32=%lu prev=%lu via_ps=%u handle=%p ps_resume=%p zw_resume=%p nt_resume=%p elapsed_ms=%llu",
            work->reason,
            work->pid,
            work->tid,
            (ULONG)primary,
        ...
        if (!NT_SUCCESS(primary)) {
            ...
            WW_LOG("TCTX system_worker_resume_fallback route=system_thread reason=%s pid=%u tid=%u status=0x%08X status_text=%s win32=%lu prev=%lu via_ps=%u handle=%p ps_resume=%p zw_resume=%p nt_resume=%p elapsed_ms=%llu",
                work->reason,
                work->pid,
                work->tid,
                (ULONG)fallback,
    ...
    VOID NTAPI worker_entry(PVOID parameter) {
        ...
        WW_LOG("TCTX system_worker_entry route=system_thread reason=%s pid=%u tid=%u set=%u mask=0x%llX caller_previous_mode=%u worker_previous_mode=%u current_pid=%p current_tid=%p process=%p thread=%p thread_type=%p thread_found=%u thread_status=0x%08X thread_status_text=%s thread_state=%lu wait_reason=%lu elapsed_ms=%llu",
            work->reason,
            work->pid,
            work->tid,
        ...
        WW_LOG("TCTX system_worker_open route=system_thread reason=%s pid=%u tid=%u desired=0x%08X status=0x%08X status_text=%s win32=%lu handle=%p thread_type=%p elapsed_ms=%llu",
            work->reason,
            work->pid,
            work->tid,
        ...
        if (NT_SUCCESS(status) && work->thread_handle) {
            if (_PsSuspendThread && (_PsResumeThread || _ZwResumeThread || ssdt_resolver::g_NtResumeThread)) {
                ...
                WW_LOG("TCTX system_worker_suspend_ps route=system_thread reason=%s pid=%u tid=%u status=0x%08X status_text=%s win32=%lu prev=%lu ps_suspend=%p ps_resume=%p elapsed_ms=%llu",
                    work->reason,
                    work->pid,
                    work->tid,
            ...
            if (!work->suspended) {
                ...
                WW_LOG("TCTX system_worker_suspend_nt route=system_thread reason=%s pid=%u tid=%u status=0x%08X status_text=%s win32=%lu prev=%lu handle=%p zw_suspend=%p nt_suspend=%p worker_previous_mode=%u elapsed_ms=%llu",
                    work->reason,
                    work->pid,
                    work->tid,
                    (ULONG)work->suspend_status,
            ...
            WW_LOG("TCTX system_worker_suspend_summary route=system_thread reason=%s pid=%u tid=%u suspended=%u via_ps=%u status=0x%08X status_text=%s win32=%lu prev=%lu thread_found=%u thread_status=0x%08X thread_status_text=%s thread_state=%lu wait_reason=%lu elapsed_ms=%llu",
                work->reason,
                work->pid,
                work->tid,
            ...
            if (work->suspended) {
                ...
                WW_LOG("TCTX system_worker_context route=system_thread reason=%s pid=%u tid=%u set=%u status=0x%08X status_text=%s win32=%lu valid=%u flags=0x%08X rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=%u dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX elapsed_ms=%llu",
                    work->reason,
                    work->pid,
                    work->tid,
                    work->input.should_set,
        ...
        if (work->thread_handle) {
            ...
            WW_LOG("TCTX system_worker_close route=system_thread reason=%s pid=%u tid=%u handle=%p status=0x%08X status_text=%s win32=%lu elapsed_ms=%llu",
                work->reason,
                work->pid,
                work->tid,
                work->thread_handle,
    ...
    NTSTATUS run(PEPROCESS process,
        ...
        if (!_PsCreateSystemThread || !_PsTerminateSystemThread || !_ObOpenObjectByPointer || !_ZwClose) {
            WW_LOG("TCTX system_route_unavailable route=system_thread reason=%s pid=%u tid=%u create=%p terminate=%p ob_open=%p zw_close=%p thread_type=%p elapsed_ms=%llu",
                reason ? reason : "none",
                request->pid,
                request->tid,
        ...
        if (!work) {
            WW_LOG("TCTX system_route_alloc_failed route=system_thread reason=%s pid=%u tid=%u bytes=%llu elapsed_ms=%llu",
                reason ? reason : "none",
                request->pid,
                request->tid,
        ...
        WW_LOG("TCTX system_route_create route=system_thread reason=%s pid=%u tid=%u set=%u status=0x%08X status_text=%s win32=%lu worker_handle=%p process=%p thread=%p caller_previous_mode=%u elapsed_ms=%llu",
            work->reason,
            work->pid,
            work->tid,
            work->input.should_set,
        ...
        if (wait_status != STATUS_SUCCESS) {
            WW_LOG("TCTX system_route_wait_bounded route=system_thread reason=%s pid=%u tid=%u wait_status=0x%08X wait_status_text=%s win32=%lu timeout_100ns=%lld worker_handle=%p elapsed_ms=%llu",
                work->reason,
                work->pid,
                work->tid,
                (ULONG)wait_status,
            ...
            if (worker_handle) {
                ...
                WW_LOG("TCTX system_route_worker_handle_close route=system_thread reason=%s pid=%u tid=%u handle=%p status=0x%08X status_text=%s win32=%lu after_timeout=1 elapsed_ms=%llu",
                    work->reason,
                    work->pid,
                    work->tid,
                    worker_handle,
        ...
        if (worker_handle) {
            NTSTATUS close_worker_status = _ZwClose(worker_handle);
            WW_LOG("TCTX system_route_worker_handle_close route=system_thread reason=%s pid=%u tid=%u handle=%p status=0x%08X status_text=%s win32=%lu elapsed_ms=%llu",
                work->reason,
                work->pid,
                work->tid,
                worker_handle,
        ...
        WW_LOG("TCTX system_route_exit route=system_thread reason=%s pid=%u tid=%u set=%u status=0x%08X status_text=%s win32=%lu create_status=0x%08X wait_status=0x%08X final_wait_status=0x%08X worker_status=0x%08X open_status=0x%08X suspend_status=0x%08X context_status=0x%08X resume_status=0x%08X resume_fallback_status=0x%08X valid=%u copied=%u rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=%u dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX elapsed_ms=%llu",
            work->reason,
            work->pid,
            work->tid,
            work->input.should_set,
...
NTSTATUS functions::handle_thread_ctx(p_thread_ctx request) {
    ...
    if (previous_mode != KernelMode && (!private_context_fast_path || debug_register_set)) {
        ...
        WW_LOG("TCTX system_route_bypass route=inline_private pid=%u tid=%u set=%u private_context_fast_path=%u private_get_available=%u private_set_available=%u private_set_without_suspend=%u previous_mode=%u requestor_mode=%u elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_set,
            private_context_fast_path ? 1u : 0u,
    ...
    WW_LOG("TCTX context_end pid=%u tid=%u set=%u private_set_without_suspend=%u status=0x%08X status_text=%s rip=0x%llX rsp=0x%llX rflags=0x%llX dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX flags=0x%08X elapsed_ms=%llu",
        request->pid,
        request->tid,
        request->should_set,
        private_set_without_suspend ? 1u : 0u,
    ...
    if (NT_SUCCESS(status) && request->should_set == 0 && !tctx_diag::user_context_sane(*request)) {
        ...
        WW_LOG("TCTX reject invalid_user_context pid=%u tid=%u status_after=0x%08X reason=%s rip=0x%llX rip_class=%s rsp=0x%llX rsp_class=%s rflags=0x%llX user_sane=0 dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr6=0x%llX dr7=0x%llX flags=0x%08X previous_mode=%u requestor_mode=%u",
            request->pid,
            request->tid,
...
namespace tsr_system_route {
    ...
    VOID NTAPI worker_entry(PVOID parameter) {
        ...
        work->worker_mode = ExGetPreviousMode();
        tctx_diag::thread_snapshot_t before = tctx_diag::query_thread_snapshot(work->pid, work->tid);
        WW_LOG("TSR system_worker_entry pid=%u tid=%u resume=%u caller_previous_mode=%u worker_previous_mode=%u current_pid=%p current_tid=%p thread=%p thread_type=%p thread_found=%u thread_status=0x%08X thread_status_text=%s thread_state=%lu wait_reason=%lu elapsed_ms=%llu",
            work->pid,
            work->tid,
            work->should_resume,
            (ULONG)work->caller_mode,
        ...
        WW_LOG("TSR system_worker_open pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu handle=%p thread_type=%p elapsed_ms=%llu",
            work->pid,
            work->tid,
            work->should_resume,
            (ULONG)work->open_status,
        ...
        if (NT_SUCCESS(work->open_status) && work->thread_handle) {
            ...
            WW_LOG("TSR system_worker_call pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu prev=%lu worker_previous_mode=%u zw_suspend=%p zw_resume=%p nt_suspend=%p nt_resume=%p elapsed_ms=%llu",
                work->pid,
                work->tid,
                work->should_resume,
        ...
        tctx_diag::thread_snapshot_t after = tctx_diag::query_thread_snapshot(work->pid, work->tid);
        WW_LOG("TSR system_worker_exit pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu open_status=0x%08X call_status=0x%08X close_status=0x%08X prev=%lu thread_after_found=%u thread_after_status=0x%08X thread_after_status_text=%s thread_after_state=%lu thread_after_wait_reason=%lu elapsed_ms=%llu",
            work->pid,
            work->tid,
            work->should_resume,
            (ULONG)work->status,
    ...
    NTSTATUS run(PETHREAD thread,
        ...
        if (!_PsCreateSystemThread || !_PsTerminateSystemThread || !_ObOpenObjectByPointer || !_ZwClose || !_ObReferenceObjectSafe || !_ObfDereferenceObject) {
            WW_LOG("TSR system_route_unavailable pid=%u tid=%u resume=%u create=%p terminate=%p ob_open=%p zw_close=%p ob_ref_safe=%p deref=%p thread_type=%p elapsed_ms=%llu",
                request->pid,
                request->tid,
        ...
        if (!_ObReferenceObjectSafe(thread)) {
            WW_LOG("TSR system_route_ref_failed pid=%u tid=%u resume=%u thread=%p elapsed_ms=%llu",
                request->pid,
                request->tid,
                request->should_resume,
                thread,
        ...
        WW_LOG("TSR system_route_create pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu worker_handle=%p caller_previous_mode=%u elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)create_status,
        ...
        NTSTATUS wait_status = KeWaitForSingleObject(&work->done, Executive, KernelMode, FALSE, &timeout);
        if (wait_status != STATUS_SUCCESS) {
            WW_LOG("TSR system_route_wait_bounded pid=%u tid=%u resume=%u wait_status=0x%08X wait_status_text=%s win32=%lu timeout_100ns=%lld worker_handle=%p elapsed_ms=%llu",
                request->pid,
                request->tid,
                request->should_resume,
                (ULONG)wait_status,
            ...
            if (worker_handle) {
                NTSTATUS close_worker_status = _ZwClose(worker_handle);
                WW_LOG("TSR system_route_worker_handle_close pid=%u tid=%u resume=%u handle=%p status=0x%08X status_text=%s win32=%lu after_timeout=1 elapsed_ms=%llu",
                    request->pid,
                    request->tid,
                    request->should_resume,
                    worker_handle,
        ...
        if (worker_handle) {
            NTSTATUS close_worker_status = _ZwClose(worker_handle);
            WW_LOG("TSR system_route_worker_handle_close pid=%u tid=%u resume=%u handle=%p status=0x%08X status_text=%s win32=%lu elapsed_ms=%llu",
                request->pid,
                request->tid,
                request->should_resume,
                worker_handle,
        ...
        WW_LOG("TSR system_route_exit pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu prev=%lu open_status=0x%08X call_status=0x%08X close_status=0x%08X elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
...
NTSTATUS functions::handle_suspend_resume_thread(p_suspend_resume_thread request) {
    if (!request || request->pid == 0 || request->tid == 0) {
        WW_LOG("TSR reject invalid_request request=%p pid=%u tid=%u resume=%u",
            request,
            request ? request->pid : 0,
            request ? request->tid : 0,
    ...
    if (KeGetCurrentIrql() != PASSIVE_LEVEL) {
        WW_LOG("TSR reject bad_irql pid=%u tid=%u resume=%u irql=%u",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)KeGetCurrentIrql());
    ...
    if (!_PsLookupProcessByProcessId || !_PsLookupThreadByThreadId || !_ObfDereferenceObject || (!use_ps && !use_handle)) {
        WW_LOG("TSR reject missing_import pid=%u tid=%u resume=%u lookup_process=%p lookup_thread=%p deref=%p use_ps=%u use_handle=%u ps_suspend=%p ps_resume=%p ob_open=%p zw_close=%p thread_type=%p",
            request->pid,
            request->tid,
            request->should_resume,
            _PsLookupProcessByProcessId,
    ...
    NTSTATUS status = _PsLookupProcessByProcessId(
        (HANDLE)(ULONG_PTR)request->pid, &process);
    if (!NT_SUCCESS(status) || !process) {
        WW_LOG("TSR lookup_process pid=%u tid=%u resume=%u status=0x%08X status_text=%s elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
    ...
    status = _PsLookupThreadByThreadId(
        (HANDLE)(ULONG_PTR)request->tid, &thread);
    if (!NT_SUCCESS(status) || !thread) {
        WW_LOG("TSR lookup_thread pid=%u tid=%u resume=%u status=0x%08X status_text=%s elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
    ...
    __try {
        PEPROCESS thread_process = IoThreadToProcess(thread);
        if (thread_process != process) {
            WW_LOG("TSR reject process_mismatch pid=%u tid=%u resume=%u process=%p thread_process=%p elapsed_ms=%llu",
                request->pid,
                request->tid,
                request->should_resume,
                process,
        ...
        status = (NTSTATUS)GetExceptionCode();
        WW_LOG("TSR reject process_check_exception pid=%u tid=%u resume=%u status=0x%08X status_text=%s elapsed_ms=%llu",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
    ...
    if (use_ps) {
        ...
        WW_LOG("TSR ps pid=%u tid=%u resume=%u status=0x%08X status_text=%s prev=%lu previous_mode=%u requestor_mode=%u exception=%u exception_code=0x%08X",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
    ...
    if ((!use_ps || !NT_SUCCESS(status)) && use_handle) {
        ...
        WW_LOG("TSR open pid=%u tid=%u resume=%u status=0x%08X handle=%p thread_type=%p",
            request->pid,
            request->tid,
            request->should_resume,
            (ULONG)status,
        ...
        if (NT_SUCCESS(status) && thread_handle) {
            ...
            WW_LOG("TSR nt pid=%u tid=%u resume=%u status=0x%08X status_text=%s prev=%lu zw_suspend=%p zw_resume=%p nt_suspend=%p nt_resume=%p previous_mode=%u requestor_mode=%u direct_nt_skipped=%u raw_ssdt_system_route=%u exception=%u exception_code=0x%08X",
                request->pid,
                request->tid,
                request->should_resume,
                (ULONG)status,
            ...
            WW_LOG("TSR open_failed pid=%u tid=%u resume=%u status=0x%08X status_text=%s use_ps=%u use_handle=%u previous_mode=%u requestor_mode=%u",
                request->pid,
                request->tid,
                request->should_resume,
                (ULONG)status,
    ...
    WW_LOG("TSR exit pid=%u tid=%u resume=%u status=0x%08X status_text=%s win32=%lu prev=%lu previous_mode=%u requestor_mode=%u thread_after_found=%u thread_after_status=0x%08X thread_after_status_text=%s thread_after_state=%lu thread_after_wait_reason=%lu elapsed_ms=%llu",
        request->pid,
        request->tid,
        request->should_resume,
        (ULONG)status,
...
NTSTATUS functions::handle_thread_query_information(p_thread_query_information request) {
    ...
    if (!_PsLookupProcessByProcessId || !_PsLookupThreadByThreadId ||
        ...
        WW_LOG("TQIF reject missing_import pid=%u tid=%u lookup_process=%p lookup_thread=%p deref=%p ob_open=%p zw_close=%p zw_query_thread=%p",
            request->pid,
            request->tid,
            _PsLookupProcessByProcessId,
            _PsLookupThreadByThreadId,
    ...
    if (!NT_SUCCESS(status) || !process) {
        ...
        WW_LOG("TQIF lookup_process pid=%u tid=%u status=0x%08X status_text=%s",
    ...
    if (!NT_SUCCESS(status) || !thread) {
        _ObfDereferenceObject(process);
        request->status = (UINT32)status;
        WW_LOG("TQIF lookup_thread pid=%u tid=%u status=0x%08X status_text=%s",
            request->pid, request->tid, (ULONG)status, tctx_diag::ntstatus_text(status));
        return status;
    }
    ...
    __try {
        ...
        if (thread_process != process) {
            ...
            WW_LOG("TQIF reject process_mismatch pid=%u tid=%u process=%p thread_process=%p",
        ...
        _ObfDereferenceObject(process);
        request->status = (UINT32)status;
        WW_LOG("TQIF reject process_check_exception pid=%u tid=%u status=0x%08X",
    ...
    request->status = (UINT32)status;
    WW_LOG("TQIF exit pid=%u tid=%u status=0x%08X status_text=%s teb=0x%llX client_pid=0x%llX client_tid=0x%llX return=%u",
        request->pid,
        request->tid,
        (ULONG)status,
        tctx_diag::ntstatus_text(status),
...
NTSTATUS functions::handle_terminate_thread(p_terminate_thread_request request) {
    ...
    if (!_PsLookupProcessByProcessId || !_PsLookupThreadByThreadId ||
        ...
        !_ZwTerminateThread) {
        request->status = (UINT32)STATUS_PROCEDURE_NOT_FOUND;
        WW_LOG("TTERM reject missing_import pid=%u tid=%u lookup_process=%p lookup_thread=%p deref=%p ob_open=%p zw_close=%p zw_term=%p",
            request->pid,
            request->tid,
            _PsLookupProcessByProcessId,
            _PsLookupThreadByThreadId,
    ...
    __try {
        ...
        if (thread_process != process) {
            ...
            _ObfDereferenceObject(process);
            request->status = (UINT32)STATUS_INVALID_CID;
            WW_LOG("TTERM reject process_mismatch pid=%u tid=%u process=%p thread_process=%p",
                request->pid, request->tid, process, thread_process);
            return STATUS_INVALID_CID;
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
    ...
    request->status = (UINT32)status;
    WW_LOG("TTERM exit pid=%u tid=%u exit=0x%08X status=0x%08X status_text=%s",
        request->pid,
        request->tid,
        request->exit_status,
        (ULONG)status,
...
NTSTATUS functions::handle_close_process_handle(p_close_handle_request request) {
    ...
    request->status = (UINT32)status;
    WW_LOG("HCLS exit pid=%u handle=0x%llX status=0x%08X status_text=%s",
        request->pid,
