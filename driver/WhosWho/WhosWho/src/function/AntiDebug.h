...
namespace anti_debug {
    ...
    __forceinline BOOLEAN check_timing_attack() {
        __try {
            // Raise IRQL to DISPATCH_LEVEL via CR8 to prevent interrupts/context
            // switches from inflating TSC measurements and causing false positives
            UINT64 old_irql = __readcr8();
            __writecr8(2); // DISPATCH_LEVEL
            ...
            __writecr8(old_irql); // Restore IRQL
            
            // Only flag if majority of trials exceeded threshold
            if (fail_count >= 2) {
                return TRUE;
            }
            
            // Variance check: detect VM timing normalization
            // (suspiciously consistent timing with too-low variance)
            UINT64 times[4];
            for (int i = 0; i < 4; i++) {
                UINT64 s = __rdtsc();
                for (int j = 0; j < 8; j++) {
