"""
AES-256-GCM encrypt the raw WhosWho.sys bytes into src/whoswho_encrypted.h
...
manual inspection still work), and then encryption proceeds. This lets
CMake skip the manual HxD export step.
...
def main():
    parser = argparse.ArgumentParser(description="Encrypt WhosWho.sys for embedding.")
    parser.add_argument(
        "--from-binary",
        dest="from_binary",
        default=None,
    ...
    lines = ["#pragma once\n", "\n"]
    lines.extend(_format_byte_array("g_whoswho_key", key))
    lines.append("\n")
    lines.extend(_format_byte_array("g_whoswho_nonce", nonce))
    lines.append("\n")
    lines.extend(_format_byte_array("g_whoswho_tag", tag))
    lines.append("\n")
    lines.extend(_format_byte_array("g_whoswho_ciphertext", ciphertext))
    lines.append("\n")
    lines.append("static const unsigned long g_whoswho_ciphertext_len = "
                 "sizeof(g_whoswho_ciphertext);\n")
