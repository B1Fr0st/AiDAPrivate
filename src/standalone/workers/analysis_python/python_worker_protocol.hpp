...
namespace aida::standalone::mcp::python_worker::wire {

constexpr std::uint32_t k_bootstrap_magic = 0x42575041U;
constexpr std::uint32_t k_frame_magic = 0x46575041U;
constexpr std::uint16_t k_protocol_version = 1;
...
struct digest_t final {
    std::array<std::uint8_t, k_digest_bytes> bytes{};

    bool empty() const noexcept {
        std::uint8_t aggregate = 0;
        for (const auto value : bytes)
            aggregate = static_cast<std::uint8_t>(aggregate | value);
        return aggregate == 0;
    }

    bool operator==(const digest_t& other) const noexcept {
        std::uint8_t difference = 0;
        for (std::size_t index = 0; index < bytes.size(); ++index)
            difference = static_cast<std::uint8_t>(difference | (bytes[index] ^ other.bytes[index]));
        return difference == 0;
    }
    ...
    std::string to_hex() const {
        ...
        for (std::size_t index = 0; index < bytes.size(); ++index) {
            value[index * 2U] = hex[bytes[index] >> 4U];
            value[index * 2U + 1U] = hex[bytes[index] & 0x0fU];
        }
        return value;
    }
};

struct session_material_t final {
...
struct frame_t final {
    frame_kind_t kind = frame_kind_t::control;
...
inline bool hash_bytes(const void* data, std::size_t size, bool hmac, const std::uint8_t* key,
                       std::size_t key_size, digest_t& output) noexcept {
    if ((!data && size != 0) || (!key && key_size != 0) || size > (std::numeric_limits<ULONG>::max)() ||
    ...
    NTSTATUS status = BCryptOpenAlgorithmProvider(&algorithm, BCRYPT_SHA256_ALGORITHM, nullptr, flags);
    ...
    status = BCryptCreateHash(algorithm, &hash, object.data(), object_size,
        hmac ? const_cast<PUCHAR>(key) : nullptr, hmac ? static_cast<ULONG>(key_size) : 0, 0);
...
inline bool sha256(const void* data, std::size_t size, digest_t& output) noexcept {
    return hash_bytes(data, size, false, nullptr, 0, output);
}

inline bool hmac_sha256(const std::uint8_t* key, std::size_t key_size, const void* data,
                        std::size_t size, digest_t& output) noexcept {
    return hash_bytes(data, size, true, key, key_size, output);
}

inline bool make_session(session_material_t& output) noexcept {
    if (!BCRYPT_SUCCESS(BCryptGenRandom(nullptr, output.nonce.data(), static_cast<ULONG>(output.nonce.size()),
            BCRYPT_USE_SYSTEM_PREFERRED_RNG)) ||
        !BCRYPT_SUCCESS(BCryptGenRandom(nullptr, output.key.data(), static_cast<ULONG>(output.key.size()),
            BCRYPT_USE_SYSTEM_PREFERRED_RNG)))
        return false;
...
inline std::array<std::uint8_t, k_bootstrap_bytes> encode_bootstrap(const session_material_t& session,
    ...
    std::memcpy(output.data() + 8 + 2U * k_digest_bytes, manifest_hash.bytes.data(), manifest_hash.bytes.size());
    return output;
}

inline bool decode_bootstrap(const std::uint8_t* data, std::size_t size, session_material_t& session,
                             digest_t& manifest_hash) noexcept {
    if (!data || size != k_bootstrap_bytes || read_u32(data) != k_bootstrap_magic ||
        read_u16(data + 4) != k_protocol_version || read_u16(data + 6) != 0)
        return false;
    ...
    std::memcpy(manifest_hash.bytes.data(), data + 8 + 2U * k_digest_bytes, manifest_hash.bytes.size());
...
inline bool write_all(HANDLE handle, const void* data, std::size_t size, DWORD& error) noexcept {
    ...
    while (size != 0) {
        const DWORD chunk = static_cast<DWORD>((std::min)(size,
...
inline bool send_frame(HANDLE handle, const session_material_t& session, frame_kind_t kind, std::uint64_t sequence,
    ...
    if ((!payload && payload_size != 0) || payload_size > maximum_payload ||
        payload_size > (std::numeric_limits<std::uint32_t>::max)() || sequence == 0) {
        error = ERROR_INVALID_PARAMETER;
        return false;
    }
    std::array<std::uint8_t, k_frame_header_bytes> header{};
    write_u32(header.data(), k_frame_magic);
    write_u16(header.data() + 4, k_protocol_version);
    ...
    try {
        ...
        authenticated.insert(authenticated.end(), header.begin(),
            header.begin() + static_cast<std::ptrdiff_t>(k_frame_header_without_tag_bytes));
        if (payload_size != 0)
    ...
    std::memcpy(header.data() + k_frame_header_without_tag_bytes, tag.bytes.data(), tag.bytes.size());
    ...
    SecureZeroMemory(authenticated.data(), authenticated.size());
    SecureZeroMemory(tag.bytes.data(), tag.bytes.size());
...
public:
    read_state_t poll(HANDLE handle, const session_material_t& session, std::uint64_t expected_sequence,
        ...
        if (!header_validated_ && !validate_header(session, expected_sequence, maximum_payload, error))
            return read_state_t::failure;
...
private:
    ...
    bool validate_header(const session_material_t& session, std::uint64_t expected_sequence,
        ...
        if (magic != k_frame_magic || version != k_protocol_version ||
            raw_kind != static_cast<std::uint16_t>(frame_kind_t::control) || sequence_ == 0 ||
    ...
    bool authenticate(const session_material_t& session, DWORD& error) {
        ...
        try {
            ...
            authenticated.insert(authenticated.end(), header_.begin(),
                header_.begin() + static_cast<std::ptrdiff_t>(k_frame_header_without_tag_bytes));
        ...
        const bool equal = hashed && constant_time_equal(tag.bytes.data(),
            header_.data() + k_frame_header_without_tag_bytes, tag.bytes.size());
        SecureZeroMemory(authenticated.data(), authenticated.size());
        SecureZeroMemory(tag.bytes.data(), tag.bytes.size());
        if (!equal) {
    ...
    frame_kind_t kind_ = frame_kind_t::control;
    bool header_validated_ = false;
    frame_failure_t failure_ = frame_failure_t::none;
};

inline digest_t protocol_hash() {
    digest_t digest;
    sha256(k_protocol_hash_material, sizeof(k_protocol_hash_material) - 1U, digest);
    return digest;
}
