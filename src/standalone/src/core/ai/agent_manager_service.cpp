	...
	operation_t operation, std::string* error)
{
	...
	loading->agents = snapshot() ? snapshot()->agents : combined_catalog();
	...
	submission.body = [cancelled, operation = std::move(operation),
		...
		try {
			...
			state = operation_state_t::failed;
			detail = "Agent Manager operation failed";
		}
		queue_completion(make_snapshot(state, label, detail));
	...
	if (!submitted.submitted) {
		...
		queue_completion(make_snapshot(operation_state_t::failed, label, detail));
