...
inline constexpr std::uint32_t k_graph_document_schema_version = 1;
...
public:
    ...
    virtual graph_kind_t supported_kind() const noexcept = 0;
    ...
    virtual graph_node_id_t deterministic_node_id(std::uint64_t address) const noexcept = 0;
    virtual graph_edge_id_t deterministic_edge_id(graph_node_id_t source,
                                                  graph_node_id_t target) const noexcept = 0;
};
...
private:
    graph_error_t fail(graph_error_code_t code, std::uint64_t subject = 0) noexcept;
    graph_error_t stale() noexcept;
    void layout_layered(std::uint64_t generation, graph_kind_t kind,
                        std::uint64_t function_address,
