...
decode_worker_pool_t::lease_hook_t pool_hook(const decode_worker_pool_t::impl_t& impl,
                                             void*& context)
{
    const auto hook = impl.hook.load(std::memory_order_acquire);
    context = impl.hook_context.load(std::memory_order_acquire);
    return hook;
}
               ...
               decode_work_item_t& item)
{
    ...
    if (victim.queue.empty())
        return false;
    item = std::move(victim.queue.front());
                      ...
                      decode_worker_pool_t::impl_t& impl)
{
    try {
        ...
        for (;;) {
            ...
            if (hook != nullptr) {
                impl.hook_active.fetch_add(1, std::memory_order_acq_rel);
                const bool lease_progress = hook(hook_context, worker.index);
                impl.hook_active.fetch_sub(1, std::memory_order_acq_rel);
                if (lease_progress)
                    continue;
...
void decode_worker_pool_t::clear_lease_hook() noexcept
{
    ...
    for (auto& worker : impl_->workers)
        worker->cv.notify_all();
    while (impl_->hook_active.load(std::memory_order_acquire) != 0)
