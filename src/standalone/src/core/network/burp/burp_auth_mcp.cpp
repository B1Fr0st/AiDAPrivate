...
tool_result_t handle_basic_decode(const json& params)
{
    ...
    if (!ok) return tool_result_t::error("invalid Basic header");
    json out;
    out["user"] = user;
    out["pass"] = pass;
    return tool_result_t::ok(out);
...
tool_result_t handle_ntlm_type3(const json& params)
{
    ...
    if (r.empty()) return tool_result_t::error(std::string("ntlm_type3 failed: ") + auth_lab::last_error());
    json out;
    out["type3_b64"] = r;
    return tool_result_t::ok(out);
}
...
tool_result_t handle_exchange_code(const json& params)
{
    ...
    if (!auth_lab::oauth2_exchange_code(te, cid, code, ru, ver, at, rt, exp))
        return tool_result_t::error(std::string("exchange failed: ") + auth_lab::last_error());
    json out;
    out["access_token"] = at;
...
tool_result_t handle_refresh(const json& params)
{
    ...
    if (!auth_lab::oauth2_refresh(te, cid, rt, at, exp))
        return tool_result_t::error(std::string("refresh failed: ") + auth_lab::last_error());
    json out;
    out["access_token"] = at;
    out["expires_in"] = exp;
    return tool_result_t::ok(out);
