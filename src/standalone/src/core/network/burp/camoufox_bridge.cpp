                                            ...
                                            std::string& state_error)
{
    ...
    diag::log_tagged_critical_fmt("camoufox",
        "bridge_health_invalidated source=%s reason=%s generation=%llu child_pid=%lu child_alive=%d exit_valid=%d exit_code=%lu exit_gle=%lu child_processes=%u browser_processes=%u process_tree=%s",
        safe_reason(source), reason.c_str(), static_cast<unsigned long long>(cleanup_generation),
                                         ...
                                         call_result_t& failure)
{
    ...
    for (;;)
    {
        ...
        else if (last_health.browser_process_count < kMinReadyBrowserProcessCount)
            reason = "browser process tree degraded";
        ...
        if (!reason.empty())
        {
            ...
            diag::log_tagged_critical_fmt("camoufox",
                "navigation_stability_failed source=%s request_id=%llu reason=%s invalidated=%d expected_generation=%llu generation=%llu expected_child_pid=%lu child_pid=%lu child_alive=%d child_processes=%u browser_processes=%u host=%s path=%s query=%d url_len=%zu stability_elapsed_ms=%llu operation_elapsed_ms=%llu process_tree=%s",
...
bool start_bridge(const launch_config_t& cfg)
{
    ...
    if (parsed.is_object())
    {
        ...
        const int browser_ready_ms_raw = json_int_or(diagnostics, "browser_ready_ms", json_int_or(parsed, "browser_ready_ms", -1));
        const int camoufox_launch_ms_raw = json_int_or(diagnostics, "camoufox_launch_ms", json_int_or(parsed, "camoufox_launch_ms", -1));
        const int diag_elapsed_ms = json_int_or(diagnostics, "elapsed_ms", json_int_or(parsed, "elapsed_ms", -1));
        const int browser_ready_ms = browser_ready_ms_raw > 0 ? browser_ready_ms_raw : diag_elapsed_ms;
        const int camoufox_launch_ms = camoufox_launch_ms_raw > 0 ? camoufox_launch_ms_raw : browser_ready_ms;
    ...
    if (!effective_cfg.headless && ready_visible.visible_window_count == 0)
    {
        ...
        diag::log_tagged_fmt("camoufox", "launch_browser visible_window_missing generation=%llu child_pid=%lu child_processes=%u browser_processes=%u process_pids=%s elapsed_ms=%llu",
            ...
            static_cast<unsigned long long>(sg().last_launch_ms));
        cleanup_client_async(failed_client, failed_pid, "launch_browser_visible_window_missing", start_generation);
        publish_state(bridge_state_t::error, sg().last_error);
        return false;
    }
    sg().state = bridge_state_t::ready;
...
bool is_ready()
{
    ...
    const bool process_tree_ready = health.browser_process_count >= kMinReadyBrowserProcessCount;
    ...
    bool publish_reused_ready = false;
    {
        ...
        if (sg().state == bridge_state_t::error && auto_restart_blocked_locked(now_ms(), blocked_reason, remaining_ms, blocked_generation))
        {
            ...
            const bool retained_ready =
                ...
                !sg().cleanup_pending &&
                health.child_alive &&
                health.browser_process_count >= kMinReadyBrowserProcessCount;
            if (retained_ready)
            {
                sg().state = bridge_state_t::ready;
                clear_error_locked();
            ...
            else if (!sg().cleanup_pending && (sg().client == nullptr || !sg().browser_open || !sg().page_verified || !sg().privacy_verified || !health.child_alive || health.browser_process_count < kMinReadyBrowserProcessCount))
            {
...
bridge_status_t get_status()
{
    ...
    if (ready_status_failed)
    {
        ...
        if (is_driver_closed_error(s.last_error))
            ...
            reason = "sidecar process exited";
        else if (s.browser_process_count < kMinReadyBrowserProcessCount)
            reason = "browser process tree degraded";
