...
enum class stack_lane_t : std::uint8_t {
    query = 0,
    overlay,
};

constexpr std::array<stack_lane_t, k_stack_tool_count> k_stack_lanes{{
    stack_lane_t::query,
    stack_lane_t::overlay,
    stack_lane_t::overlay,
}};
...
protocol::mcp_result_t stack_handlers_t::dispatch(
    ...
    const auto lane = k_stack_lanes.at(index);
    ...
    try {
        ...
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_input,
            "Stack backend arguments cannot be serialized.",
            protocol::json{{"phase", "stack_backend_serialization"}});
    }
    ...
    auto adapter_result = [&]() -> adapter_result_t<adapter_response_t> {
        switch (lane) {
        case stack_lane_t::query:
            return workspace_.query(name, request);
        case stack_lane_t::overlay:
    ...
    if (cancellation.cancelled()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::cancelled,
            "Stack tool invocation was cancelled during adapter execution.",
            protocol::json{{"phase", "stack_post_adapter"}});
    }
    ...
    if (structured.is_discarded() || !structured.is_object()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_output,
            "Stack adapter returned malformed structured output.",
            protocol::json{{"phase", "stack_output_parse"},
                           {"response_bytes", response.payload.size()}});
    }
    if (cancellation.cancelled()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::cancelled,
            "Stack tool invocation was cancelled before output validation.",
            protocol::json{{"phase", "stack_pre_output_validation"}});
    }
...
protocol::mcp_result_t stack_frame(const handlers::stack_handlers_t& handlers,
    ...
    return handlers.invoke("stack_frame", arguments, cancellation, aida_metadata);
}
...
protocol::mcp_result_t declare_stack(const handlers::stack_handlers_t& handlers,
                                    ...
                                    const protocol::cancellation_token_t& cancellation,
                                    const protocol::json& aida_metadata) {
    return handlers.invoke("declare_stack", arguments, cancellation, aida_metadata);
}
...
protocol::mcp_result_t delete_stack(const handlers::stack_handlers_t& handlers,
                                    ...
                                    const protocol::cancellation_token_t& cancellation,
                                    const protocol::json& aida_metadata) {
    return handlers.invoke("delete_stack", arguments, cancellation, aida_metadata);
}
