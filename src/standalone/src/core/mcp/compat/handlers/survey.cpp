...
void validate_generated_descriptor(const contract_descriptor_t& descriptor,
    ...
    if (descriptor.name != name || descriptor.adapter_symbol != expected_adapter ||
        ...
        throw std::runtime_error(
            "generated survey descriptor policy mismatch for " + std::string(name));
    }
}
...
validation_failure_t enum_member(const json& object, std::string_view field,
    ...
    if (value.size() > 32U) {
        return exceeded_value(path, 32ULL, static_cast<std::uint64_t>(value.size()));
    }
...
validation_failure_t validate_tool_bounds(const json& arguments,
                                         const survey_handler_limits_t& limits) {
    if (auto failure = validate_routing_bounds(arguments, limits)) {
        return failure;
    }
    if (auto failure = enum_member(
            arguments, "detail_level", {}, {"standard", "minimal"})) {
        return failure;
    }
    return std::nullopt;
}

json normalized_backend_arguments(const json& arguments) {
    json normalized = arguments;
    normalized.erase("pid");
    normalized.erase("bin_name");
    if (!normalized.contains("detail_level")) {
        normalized["detail_level"] = "standard";
    }
    return normalized;
}
...
mcp_result_t adapter_failure(const adapter_error_t& error) {
    return mcp_result_t::failure(
        ...
        "Survey workspace adapter rejected the request.",
        json{
...
survey_handlers_t::survey_handlers_t(workspace_adapter_t& workspace,
    ...
    for (std::size_t index = 0; index < k_survey_names.size(); ++index) {
        const auto name = k_survey_names[index];
        const auto* descriptor =
            aida::standalone::mcp::compat::find_contract(name);
        if (descriptor == nullptr) {
            throw std::runtime_error(
                "generated survey descriptor is missing for " + std::string(name));
        }
        ...
        if (!validation.valid) {
            throw std::runtime_error(
                "generated survey contract validation failed for " + std::string(name) +
                ": " + validation.reason);
        }
    }
}
...
protocol::mcp_result_t survey_handlers_t::dispatch(
    ...
    if (cancellation.cancelled()) {
        return protocol::mcp_result_t::failure(
            ...
            "Survey tool invocation was cancelled before adapter routing.",
            protocol::json{{"phase", "survey_pre_route"}});
    }
    ...
    if (serialized_arguments.size() > limits_.max_request_bytes) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_input,
            "Survey tool request exceeds the bounded adapter quota.",
            exceeded_value(
                "request_bytes",
                static_cast<std::uint64_t>(limits_.max_request_bytes),
                static_cast<std::uint64_t>(serialized_arguments.size())));
    }
    if (auto failure = validate_tool_bounds(arguments, limits_)) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_input,
            "Survey tool arguments violate the bounded adapter policy.",
            *failure);
    }
    ...
    protocol::json backend_arguments = normalized_backend_arguments(arguments);
    try {
        request.payload = backend_arguments.dump();
    } catch (const std::exception&) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_input,
            "Survey backend arguments cannot be serialized.",
            protocol::json{{"phase", "survey_backend_serialization"}});
    }
    request.deadline = std::chrono::steady_clock::now() +
        execution_timeout(contract, limits_.max_execution_time);

    auto adapter_result = workspace_.query(name, request);
    if (!adapter_result) {
    ...
    if (cancellation.cancelled()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::cancelled,
            "Survey tool invocation was cancelled during adapter execution.",
            protocol::json{{"phase", "survey_post_adapter"}});
    }
    ...
    if (structured.is_discarded() || !structured.is_object()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::invalid_output,
            "Survey adapter returned malformed structured output.",
            protocol::json{{"phase", "survey_output_parse"},
                           {"response_bytes", response.payload.size()}});
    }
    if (cancellation.cancelled()) {
        return protocol::mcp_result_t::failure(
            protocol::result_error_code_t::cancelled,
            "Survey tool invocation was cancelled before output validation.",
            protocol::json{{"phase", "survey_pre_output_validation"}});
    }
