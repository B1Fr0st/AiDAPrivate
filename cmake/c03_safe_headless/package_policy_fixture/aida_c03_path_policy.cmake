...
function(aida_c03_validate_canonical_regular_file output_path output_sha256 input_path descriptor)
    if(NOT ARGC EQUAL 4)
        message(FATAL_ERROR "AiDA C03 canonical regular-file validation arguments are malformed")
    endif()
    ...
    if(NOT EXISTS "${input_path}" OR IS_DIRECTORY "${input_path}" OR IS_SYMLINK "${input_path}")
        message(FATAL_ERROR "AiDA C03 ${descriptor} must be an existing non-reparse regular file: ${input_path}")
    endif()
    ...
    if(NOT input_path STREQUAL _aida_c03_real_path)
        message(FATAL_ERROR "AiDA C03 ${descriptor} path case, spelling, or final identity is noncanonical: ${input_path}")
    endif()
...
function(aida_c03_validate_canonical_directory output_path input_path descriptor)
    if(NOT ARGC EQUAL 3)
        message(FATAL_ERROR "AiDA C03 canonical directory validation arguments are malformed")
    endif()
    _aida_c03_validate_raw_windows_path_shape("${input_path}" "${descriptor}")
    if(NOT EXISTS "${input_path}" OR NOT IS_DIRECTORY "${input_path}" OR IS_SYMLINK "${input_path}")
        message(FATAL_ERROR "AiDA C03 ${descriptor} must be an existing non-reparse directory: ${input_path}")
    endif()
    file(REAL_PATH "${input_path}" _aida_c03_real_path)
    file(TO_CMAKE_PATH "${_aida_c03_real_path}" _aida_c03_real_path)
    if(NOT input_path STREQUAL _aida_c03_real_path)
...
function(aida_c03_validate_no_reparse_chain powershell_executable policy_script input_path descriptor)
    if(NOT ARGC EQUAL 4)
        message(FATAL_ERROR "AiDA C03 reparse-chain validation arguments are malformed")
    endif()
    ...
    if(NOT _aida_c03_result EQUAL 0 OR NOT _aida_c03_final_path STREQUAL input_path)
        message(FATAL_ERROR "AiDA C03 ${descriptor} contains a reparse transition or noncanonical final spelling: ${input_path} ${_aida_c03_error}")
    endif()
endfunction()
