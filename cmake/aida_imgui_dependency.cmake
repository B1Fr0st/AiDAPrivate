...
function(aida_configure_imgui_dependency source_dir)
    if(NOT EXISTS "${AIDA_IMGUI_LOCK_FILE}")
        message(FATAL_ERROR "AiDA Dear ImGui dependency lock is missing")
    endif()
    ...
    if(NOT _aida_imgui_schema EQUAL 1 OR
       ...
       NOT _aida_imgui_multi_viewport STREQUAL "disabled")
        message(FATAL_ERROR "AiDA Dear ImGui dependency lock does not match the approved docking authority")
    endif()
    string(JSON _aida_imgui_file_count LENGTH "${_aida_imgui_lock}" files)
    if(NOT _aida_imgui_file_count EQUAL 17)
        message(FATAL_ERROR "AiDA Dear ImGui dependency inventory cardinality is invalid")
    endif()
    ...
    foreach(_aida_imgui_index RANGE 0 ${_aida_imgui_file_last})
        ...
        if(_aida_imgui_path MATCHES "(^|/)\\.\\.(/|$)" OR
           ...
           NOT _aida_imgui_sha256 MATCHES "^[0-9a-f]+$")
            message(FATAL_ERROR "AiDA Dear ImGui dependency inventory contains an invalid path or identity")
        endif()
        ...
        if(NOT EXISTS "${_aida_imgui_absolute}" OR IS_DIRECTORY "${_aida_imgui_absolute}")
            message(FATAL_ERROR "AiDA Dear ImGui dependency file is missing: ${_aida_imgui_path}. Run tools/bootstrap_imgui_dependency.ps1")
        endif()
        file(SIZE "${_aida_imgui_absolute}" _aida_imgui_actual_size)
        ...
        if(NOT _aida_imgui_actual_size EQUAL _aida_imgui_size OR
           NOT _aida_imgui_actual_sha256 STREQUAL _aida_imgui_sha256)
            message(FATAL_ERROR "AiDA Dear ImGui dependency identity mismatch: ${_aida_imgui_path}. Run tools/bootstrap_imgui_dependency.ps1")
        endif()
        if(_aida_imgui_path STREQUAL "imgui.cpp")
            set(_aida_imgui_source_sha256 "${_aida_imgui_actual_sha256}")
        endif()
    endforeach()
    if(NOT _aida_imgui_source_sha256 STREQUAL AIDA_IMGUI_EXPECTED_SOURCE_SHA256)
        message(FATAL_ERROR "AiDA Dear ImGui source fingerprint does not match the approved docking authority")
    endif()
    ...
    if(NOT _aida_imgui_public_header MATCHES "#define[ \t]+IMGUI_VERSION_NUM[ \t]+19210" OR
       ...
       NOT _aida_imgui_internal_header MATCHES "DockBuilderFinish")
        message(FATAL_ERROR "AiDA Dear ImGui dependency lacks the approved docking capabilities")
    endif()
